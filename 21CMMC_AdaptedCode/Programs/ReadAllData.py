...
if __name__ == '__main__':
	...
	# 3) Provide the directory name for the stored data (21CMMC.py will create the directory with a timestamp)
        KeepDataString = 'KEEP_MCMC_DATA_Tue_06_Aug_2019_11h_35m_59s'
	...
	FileNames = 'LF_taue_HERA331_Co-eval_multiz'
	...
	f = open('ReionModel_%sburnin.out'%(FileNames),'r')
	parameters_burnin = [[float(v) for v in line.rstrip('\n').split('\t')] for line in f.readlines()]
	f.close()

	f2 = open('ReionModel_%sburninprob.out'%(FileNames),'r')
	probs_burnin = [float(v) for v in f2.readlines()]
	f2.close()

	f3 = open('ReionModel_%sNFvalsburnin.out'%(FileNames),'r')
	...
	f = open('ReionModel_%s.out'%(FileNames),'r')
	parameters = [[float(v) for v in line.rstrip('\n').split('\t')] for line in f.readlines()]
	f.close()

	f2 = open('ReionModel_%sprob.out'%(FileNames),'r')
	probs = [float(v) for v in f2.readlines()]
	f2.close()

	f3 = open('ReionModel_%sNFvals.out'%(FileNames),'r')
