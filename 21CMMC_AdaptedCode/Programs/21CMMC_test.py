...
from CosmoHammer_21CMMC.likelihood.module.Likelihood21cmFast_test import Likelihood21cmFast_multiz

from CosmoHammer_21CMMC.likelihood.module.Likelihood21cmFast_test import McGreer_Redshift
from CosmoHammer_21CMMC.likelihood.module.Likelihood21cmFast_test import QSO_Redshift

def voronoi_volumes(v):
    #v = Voronoi(points,qhull_options='QJ')
    vol = numpy.zeros(v.npoints)
...
# Be careful that you are using a valid interpolation table (i.e cosmology etc. hasn't changed)
	...
	if IncludeLightCone is False:
		...
		Redshift = ['8.000000','9.000000', '10.000000']
		#Redshift = ['6.000000','6.500000', '7.000000']
                HII_DIMS = 128
                MPC_DIMS = 256
...
#		Redshift = ['6.000594']
	...
	# A check to see if additional redshifts have been added to the computation that are not necessary
        IncludeMorphologyHII = True
        IncludePoissonBubHII = True
        IncludeMorphologyHI = False
        IncludePoissonBubHI = False
        IncludePS = False
...
#	PriorLegend['GazagnesPrior'] = IncludeGazagnes
...
#		MockObsFileName = 'MockObs_128_256'
...
#		ModelName = 'BrightGalaxies'
		...
		if IncludeLightCone is True:
			...
			# If Include_Ts_fluc is set, the user must ensure that the co-eval redshift to be sampled (set by the Redshift list above) is to be sampled by the code.			
                                MockObsDirectory = 'MockObs/mock_new'
			...
			#	BoxType = 'Co-Eval'			
				...
				for i in range(len(Redshift)):		
					mockobs_k_values = numpy.loadtxt('%s/%s%s.txt'%(MockObsDirectory, MockObsPsFileName,Redshift[i]), usecols=(0,))
					mockobs_PS_values = numpy.loadtxt('%s/%s%s.txt'%(MockObsDirectory, MockObsPsFileName,Redshift[i]), usecols=(1,))
	...
	if not IncludeLF is 2:
      ...
      #  multi_z_mockobs_hii_bins = []
        multi_z_mockobs_hii_data = []
        multi_z_mockobs_hii_vor = []
        multi_z_mockobs_hii_vol = []
        lim1 = -100
        lim2 = 100
        vol_lim= 10

        if IncludeMorphologyHII or IncludePoissonBubHII:
                for i in range(len(Redshift)):
                        f = open('%s/obs_hii_z%s_vol.bin'%(MockObsDirectory,Redshift[i]), "r")
                        volume = numpy.fromfile(f, dtype=numpy.uint64)*2
                        
                        f = open('%s/obs_hii_z%s_elong.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_e = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/obs_hii_z%s_flat.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_f = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/obs_hii_z%s_spars.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_s = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/obs_hii_z%s_ncomp.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_n = numpy.fromfile(f, dtype=numpy.float)
                        good = volume > vol_lim
                        volume = volume[good]
                        all_e = all_e[good]
                        all_f = all_f[good]
                        all_s = all_s[good]
                        all_n = all_n[good]                        
                        data = zip(numpy.log10(volume), all_s, all_e, all_f)
                        seen = set()
                        uniq = []
                        for x in data:
                                if x not in seen:
                        ...
                        data=list(uniq)
                        data.extend([[-1,lim1,lim1,lim1], [-1,lim2,lim1,lim1], [-1,lim1,lim1,lim2], [-1,lim1,lim2,lim1]])
                        data.extend([[-1,lim2,lim2,lim1], [-1,lim2,lim2,lim2], [-1,lim1,lim2,lim2], [-1,lim2,lim1,lim2]])
                        data.extend([[16,lim1,lim1,lim1], [16,lim2,lim1,lim1], [16,lim2,lim2,lim1],[16,lim2,lim2,lim2],[16,lim1,lim2,lim2], [16,lim1,lim1,lim2],[16,lim2,lim1,lim2],[16,lim1,lim2,lim1]])

                        data_vor = Voronoi(data, qhull_options='QJ')
                        data_vol = voronoi_volumes(data_vor)
                        multi_z_mockobs_hii_data.append(data)
                        multi_z_mockobs_hii_vor.append(data_vor)
                        multi_z_mockobs_hii_vol.append(data_vol)
              

        multi_z_mockobs_hi_data = []
        multi_z_mockobs_hi_vor = []
        multi_z_mockobs_hi_vol = []
        lim1 = -100
        lim2 = 100
        vol_lim= 10
        if IncludeMorphologyHI or IncludePoissonBubHI:
                for i in range(len(Redshift)):
                        f = open('%s/stats/obs_hi_z%s_vol.bin'%(MockObsDirectory,Redshift[i]), "r")
                        volume = numpy.fromfile(f, dtype=numpy.uint64)*2
                        
                        f = open('%s/stats/obs_hi_z%s_elong.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_e = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/stats/obs_hi_z%s_flat.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_f = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/stats/obs_hi_z%s_spars.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_s = numpy.fromfile(f, dtype=numpy.float)
                        
                        f = open('%s/stats/obs_hi_z%s_ncomp.bin'%(MockObsDirectory,Redshift[i]), "r")
                        all_n = numpy.fromfile(f, dtype=numpy.float)
                        ...
                        data = zip(numpy.log10(volume), all_e, all_f, all_s)
                        seen = set()
                        uniq = []
                        for x in data:
                                if x not in seen:
                        ...
                        data_vor = Voronoi(data, qhull_options='QJ')
                        data_vol = voronoi_volumes(data_vor)
                        multi_z_mockobs_hi_data.append(data)
                        multi_z_mockobs_hi_vor.append(data_vor)
                        multi_z_mockobs_hi_vol.append(data_vol)        # Redshift list to compute Luminosity functions.
	#if IncludeLF is True:
	if IncludeLF:
	...
	ObsDuration = '1000hr'
        ...
        NoiseDirectory = 'NoiseData/Morph_SKA_128/bin_ind_hii/'
    ...
    #    part = ['p1','p2','p3']
        ...
        if IncludeMorphologyHII is True or IncludeMorphologyHI is True or IncludePoissonBubHII is True or IncludePoissonBubHI is True:
                for i in range(len(Redshift)):
                        uv_maps.append(numpy.loadtxt('NoiseData/UVmap_SKA/128/uv_map_z%s.txt'%(Redshift[i]), ndmin=2))
...
#		ErrorFileName = 'TotalError_BrightGalaxies_%s_%s'%(Telescope_Name,ObsDuration)
		...
		if IncludeLightCone is True:
			...
			if not IncludeLF is 2:
				...
				for i in range(len(Redshift)):
                                        NoiseFileName = 'PS_%s_128/Total_PS_error_z%s.txt'%(Telescope_Name,Redshift[i])
					Error_k_values = numpy.loadtxt('NoiseData/%s'%(NoiseFileName), usecols=(0,))
					Error_PS_values = numpy.loadtxt('NoiseData/%s'%(NoiseFileName), usecols=(1,))
	...
	Likelihoodmodel21cmFast = Likelihood21cmFast_multiz(Redshifts_For_LF,multi_z_obs_Muv,multi_z_obs_phi,multi_z_obs_Error_phi,multi_z_mockobs_k,multi_z_mockobs_PS,multi_z_Error_k,multi_z_Error_PS,Redshift,Redshifts_For_Prior,param_legend,Fiducial_Params,FlagOptions,param_string_names,NSplinePoints,TsCalc_z,foreground_cut,shot_noise_cut,IncludeLightCone,IncludeLF,ModUncert,PriorLegend,NFVals_QSODamping,PDFVals_QSODamping, IncludePS, IncludeMorphologyHII,IncludeMorphologyHI ,IncludePoissonBubHII,IncludePoissonBubHI, HII_DIMS, MPC_DIMS, multi_z_mockobs_hii_data, multi_z_mockobs_hii_vol, multi_z_mockobs_hii_vor, multi_z_mockobs_hi_data, multi_z_mockobs_hi_vol, multi_z_mockobs_hi_vor, uv_maps)	

	chain.addLikelihoodModule(Likelihoodmodel21cmFast)

	chain.setup()

	File_String = 'ReionModel_Voronoi_DiffBox_test_%s_%s'%(Telescope_Name,multiz_flag)
